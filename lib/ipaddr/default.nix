...
let
  ...
  parseIPv4 = s:
    let
      ...
      octets = map pkgs.lib.toInt (parsedIPv4Addr parse);
      suffix =
        let
        ...
        in
          ...
          then []
          else map (x: pkgs.lib.toInt (pkgs.lib.removePrefix "/" x)) suffix';
    in
      if (parse != [])              &&
         (pkgs.lib.all (x: x <= 255) octets) &&
         (pkgs.lib.all (x: x <= 32) suffix)
      then octets ++ suffix
      else [];
  ...
  parseIPv4RFC1918 = s:
    ...
    in
      ...
      then []
        ...
        let
          suffix = parsedIPv4PrefixLength parse;
          octet1 = pkgs.lib.elemAt parse 0;
          octet2 = pkgs.lib.elemAt parse 1;
          block1 = octet1 == 10 && (suffix == [] || pkgs.lib.head suffix >= 8);
          block2 = octet1 == 172 && (octet2 >= 16 && octet2 < 32) && (suffix == [] || pkgs.lib.head suffix >= 12);
          block3 = octet1 == 192 && octet2 == 168 && (suffix == [] || pkgs.lib.head suffix >= 16);
        in
  ...
  parsedIPv4Addr = pkgs.lib.take 4;
  parsedIPv4PrefixLength = pkgs.lib.drop 4;
  ...
  unparseIPv4 = l:
    ...
    in
      if (pkgs.lib.length l < 4)                     ||
         (pkgs.lib.length l > 5)                     ||
         (pkgs.lib.any (x: x < 0 || x > 255) octets) ||
         (pkgs.lib.any (x: x < 0 || x > 32) suffix)
      then ""
      else
        let
          addr = pkgs.lib.concatMapStringsSep "." toString octets;
          suffix' = pkgs.lib.concatMapStrings toString suffix;
        in
          if suffix' == ""
          then addr
          else pkgs.lib.concatStringsSep "/" [ addr suffix' ];
  ...
  parseIPv6 = s:
    let
      ...
      parse = if good == null then [] else pkgs.lib.take 1 good;
      suffix = if parse == [] then [] else parsedIPv6PrefixLength parse;
    in
      if (suffix == [])
      then parse
      else
        if (pkgs.lib.head suffix <= 128) then parse else [];
  ...
  parsedIPv6PrefixLength = l:
    let
      addr = pkgs.lib.head l;
      suffix = pkgs.lib.tail (pkgs.lib.splitString "/" addr);
    in
      if suffix == [] then [] else map pkgs.lib.toInt suffix;

  parsedIPv6Addr = l:
    let
      addr = pkgs.lib.head l;
    in
      pkgs.lib.head (pkgs.lib.splitString "/" addr);

  unparseIPv6 = l: if l == [] then "" else pkgs.lib.head l;
  ...
  ipv4AddrFromCIDR = s:
    assert isIPv4CIDR s;
    pkgs.lib.head (pkgs.lib.splitString "/" s);
  ...
  prefixLengthFromCIDR = s:
    ...
    pkgs.lib.toInt (pkgs.lib.head (pkgs.lib.tail (pkgs.lib.splitString "/" s)));
